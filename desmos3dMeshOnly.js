// ==UserScript==
// @name         Desmos Three.js Mesh Only
...
(function() {
      ...
      for (let i = meshData.length; i < numMeshes; i++) {
        ...
        for (let c = 0; c < 3; c++) {
          const helper = Calc.HelperExpression({
            latex: `\\xi_{mesh${"XYZ"[c]}${i}}`
          });
          helper.observe('listValue', (_, {
            listValue
          }) => {
  ...
  function observeLights() {
    ...
    for (let c = 0; c < 3; c++) {
      Calc.HelperExpression({
          latex: `\\xi_{light${"XYZ"[c]}}`
        })
        .observe('numericValue', (_, {
          numericValue
        }) => updateLightPos(c, numericValue))
    }
    Calc.HelperExpression({
        latex: `\\xi_{lightIntensity}`
      })
      .observe('numericValue', (_, {
        numericValue
      }) => updateLightIntensity(numericValue))
    Calc.HelperExpression({
        latex: `\\xi_{ambientLightIntensity}`
      })
      .observe('numericValue', (_, {
        numericValue
      }) => updateAmbientLightIntensity(numericValue))
  }
